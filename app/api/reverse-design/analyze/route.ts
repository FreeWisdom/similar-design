...
export async function POST(request: NextRequest) {
  try {
    ...
    if (authError || !user) {
      return NextResponse.json({ error: '需要登录后才能使用设计反推功能' }, { status: 401 });
    }
    ...
    if (files.length > MAX_FILES) {
      return NextResponse.json({ error: `一次最多上传 ${MAX_FILES} 张图片` }, { status: 400 });
    }

    for (const f of files) {
      const sizeMb = (f.size || 0) / (1024 * 1024);
      if (sizeMb > MAX_SIZE_MB) {
        return NextResponse.json({ error: `图片 ${f.name || ''} 超过 ${MAX_SIZE_MB}MB 大小限制` }, { status: 400 });
      }
      if (!f.type?.startsWith('image/')) {
        return NextResponse.json({ error: `文件 ${f.name || ''} 不是有效图片类型` }, { status: 400 });
      }
    }
...
- 仅输出 JSON，不要任何解释、Markdown 或额外文本。`;
    ...
    const messageContent: any[] = [
      { type: 'input_text', text: `${userIntro}\n\n输出格式要求：\n${expectedSchema}` },
      // 依次附上多张图片
    ...
    if (!content) {
      return NextResponse.json({ error: '模型无响应，请稍后重试' }, { status: 502 });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: 'AI 返回数据解析失败', details: e instanceof Error ? e.message : String(e) }, { status: 500 });
    }
