...
export async function POST(request: NextRequest) {
  try {
    ...
    // 组合多图输入与中文系统提示
    const systemPrompt = `你是资深 UI/UX 设计审稿专家与提示词工程专家。你的目标是从知识卡片图片中提炼出可复用、可落地的设计规范，并将其转化为稳定、可执行、低发散的生成型提示词模板。请避免华丽描述与自由发挥，优先输出可执行的参数与规则（网格、尺寸、字号、间距、色值、层级、对齐、阴影、图标风格、插画处理、安全区等），并指出变量与可变范围以保证多图一致性。`;
//     `你是资深 UI/UX 设计审稿专家与提示词工程专家。你的任务：
...
// - 统一多图的共性风格，指出差异化元素与可变参数，避免逐图重复罗列。`;
    ...
    const userIntro = `请根据我提供的知识卡片图片，完成以下任务：
1. **设计风格分析**
   - 从图片中系统分析整体设计语言：布局/网格、信息层级、排版、配色、留白、对齐、对比、图标或插画风格、组件与交互动效暗示、阴影与质感、模块编排等。
   - 分析内容结构：主标题、副标题、核心观点、数据点、支撑说明、图表可视化、字体层级、信息密度。
   - 提炼多图的共性风格，并指出差异化元素与可变参数（避免逐图重复罗列）。

2. **生成通用提示词**
   请将上述分析总结成一个可直接用于 AI 生成知识卡片的提示词模板，确保提示词中包含：
   - 整体风格（核心设计原则）
   - 整体配色方案（背景/文字的主辅色及应用场景）
   - 整体排版方案（字体风格、层级体系、字号、行距、字间距）
   - 卡片尺寸
   - 布局方式（网格列数、间距、构图、模块编排）
   - 间距与节奏（留白、韵律、分割线规则）
   - 阴影与深度设计（阴影、层级、质感处理）
   - 图片与图标（插画/图形风格、图标风格）
   - 标题设计（主/副标题、颜色）
   - 内容结构（核心观点、数据支撑、补充说明）
   - 视觉元素（背景装饰、图表、高亮元素、留白处理）
   - 文字规范（主要文字、强调文字、字体层级、多语言混排）
   - 输出格式要求（参考所给图片总结）

3. **输出格式**
   - 先输出【分析报告】：按上述分析维度分点说明
   - 再输出【通用提示词模板】：用自然语言组合上述要点，保持结构清晰
   - 提示词中不要出现具体的图片数量描述`
    // `
